...
int nvglCreateImageFromHandleGL2(NVGcontext *ctx, GLuint textureId, int w,
                                 int h, int flags);
...
int nvglCreateImageFromHandleGLES2(NVGcontext *ctx, GLuint textureId, int w,
                                   int h, int flags);
...
int nvglCreateImageFromHandleGLES3(NVGcontext *ctx, GLuint textureId, int w,
                                   int h, int flags);
...
static GLNVGtexture *glnvg__allocTexture(GLNVGcontext *gl) {
    ...
    if (tex == NULL) {
        if (gl->ntextures + 1 > gl->ctextures) {
            ...
            textures = (GLNVGtexture *)realloc(
                gl->textures, sizeof(GLNVGtexture) * ctextures);
            if (textures == NULL)
                return NULL;
...
static int glnvg__renderCreate(void *uptr) {
    ...
    // https://github.com/memononen/nanovg/issues/46
    ...
    static const char *fillFragShader =
        ...
        "	#define scissorMat mat3(frag[0].xyz, frag[1].xyz, "
        "frag[2].xyz)\n"
        ...
        "#endif\n"
        "		if (texType == 1) color = "
        "vec4(color.xyz*color.w,color.w);"
...
static int glnvg__renderCreateTexture(void *uptr, int type, int w, int h,
    ...
    if (glnvg__nearestPow2(w) != (unsigned int)w ||
        ...
        if ((imageFlags & NVG_IMAGE_REPEATX) != 0 ||
            ...
            printf("Repeat X/Y is not supported for non power-of-two textures "
                   "(%d x %d)\n",
                   w, h);
        ...
        if (imageFlags & NVG_IMAGE_GENERATE_MIPMAPS) {
            printf("Mip-maps is not support for non power-of-two textures (%d "
                   "x %d)\n",
                   w, h);
...
#if defined(NANOVG_GL2)
    // GL 1.4 and later has support for generating mipmaps using a tex
    // parameter.
    if (imageFlags & NVG_IMAGE_GENERATE_MIPMAPS) {
...
static int glnvg__convertPaint(GLNVGcontext *gl, GLNVGfragUniforms *frag,
    ...
    if (paint->image != 0) {
        ...
        if (tex->type == NVG_TEXTURE_RGBA)
            frag->texType =
                (tex->flags & NVG_IMAGE_PREMULTIPLIED) ? 0.0f : 1.0f;
        else
            frag->texType = 2.0f;
#endif
...
static void glnvg__renderFlush(void *uptr) {
    ...
    if (gl->ncalls > 0) {
        ...
        glBufferData(GL_UNIFORM_BUFFER, gl->nuniforms * gl->fragSize,
...
static GLNVGcall *glnvg__allocCall(GLNVGcontext *gl) {
    ...
    if (gl->ncalls + 1 > gl->ccalls) {
        GLNVGcall *calls;
        int ccalls = glnvg__maxi(gl->ncalls + 1, 128) +
                     gl->ccalls / 2; // 1.5x Overallocate
...
static int glnvg__allocPaths(GLNVGcontext *gl, int n) {
    ...
    if (gl->npaths + n > gl->cpaths) {
        GLNVGpath *paths;
        int cpaths = glnvg__maxi(gl->npaths + n, 128) +
                     gl->cpaths / 2; // 1.5x Overallocate
...
static int glnvg__allocVerts(GLNVGcontext *gl, int n) {
    ...
    if (gl->nverts + n > gl->cverts) {
        NVGvertex *verts;
        int cverts = glnvg__maxi(gl->nverts + n, 4096) +
                     gl->cverts / 2; // 1.5x Overallocate
