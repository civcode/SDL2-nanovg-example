...
void nvgBeginFrame(NVGcontext *ctx, int windowWidth, int windowHeight,
...
int nvgCreateImageRGBA(NVGcontext *ctx, int w, int h, int imageFlags,
    ...
    return ctx->params.renderCreateTexture(
        ctx->params.userPtr, NVG_TEXTURE_RGBA, w, h, imageFlags, data);
}

void nvgUpdateImage(NVGcontext *ctx, int image, const unsigned char *data) {
    int w, h;
    ctx->params.renderGetTextureSize(ctx->params.userPtr, image, &w, &h);
    ctx->params.renderUpdateTexture(ctx->params.userPtr, image, 0, 0, w, h,
                                    data);
}
...
void nvgGlobalCompositeBlendFunc(NVGcontext *ctx, int sfactor, int dfactor) {
    nvgGlobalCompositeBlendFuncSeparate(ctx, sfactor, dfactor, sfactor,
                                        dfactor);
}
...
static void nvg__appendCommands(NVGcontext *ctx, float *vals, int nvals) {
    ...
    while (i < nvals) {
        ...
        switch (cmd) {
        case NVG_MOVETO:
            nvgTransformPoint(&vals[i + 1], &vals[i + 2], state->xform,
                              vals[i + 1], vals[i + 2]);
            i += 3;
            break;
        case NVG_LINETO:
        ...
        case NVG_BEZIERTO:
            nvgTransformPoint(&vals[i + 1], &vals[i + 2], state->xform,
                              vals[i + 1], vals[i + 2]);
            nvgTransformPoint(&vals[i + 3], &vals[i + 4], state->xform,
                              vals[i + 3], vals[i + 4]);
            nvgTransformPoint(&vals[i + 5], &vals[i + 6], state->xform,
                              vals[i + 5], vals[i + 6]);
            i += 7;
            break;
        case NVG_CLOSE:
...
static NVGvertex *nvg__allocTempVerts(NVGcontext *ctx, int nverts) {
    if (nverts > ctx->cache->cverts) {
        NVGvertex *verts;
        int cverts =
            (nverts + 0xff) & ~0xff; // Round up to prevent allocations when
                                     // things change just slightly.
        verts =
            (NVGvertex *)realloc(ctx->cache->verts, sizeof(NVGvertex) * cverts);
        if (verts == NULL)
            return NULL;
    ...
    while (i < ctx->ncommands) {
        ...
        case NVG_BEZIERTO:
            ...
            if (last != NULL) {
                ...
                p = &ctx->commands[i + 5];
                nvg__tesselateBezier(ctx, last->x, last->y, cp1[0], cp1[1],
        ...
        for (j = 0; j < path->count; j++) {
            ...
            if (p1->flags & NVG_PT_CORNER) {
                if ((dmr2 * miterLimit * miterLimit) < 1.0f ||
                    lineJoin == NVG_BEVEL || lineJoin == NVG_ROUND) {
...
static int nvg__expandStroke(NVGcontext *ctx, float w, int lineCap,
    ...
    for (i = 0; i < cache->npaths; i++) {
        ...
        if (lineJoin == NVG_ROUND)
            ...
            cverts +=
                (path->count + path->nbevel * 5 + 1) * 2; // plus one for loop
        if (loop == 0) {
            // space for caps
    ...
    for (i = 0; i < cache->npaths; i++) {
        ...
        for (j = s; j < e; ++j) {
            if ((p1->flags & (NVG_PT_BEVEL | NVG_PR_INNERBEVEL)) != 0) {
                ...
                nvg__vset(dst, p1->x + (p1->dmx * w), p1->y + (p1->dmy * w), 0,
                          1);
                dst++;
                nvg__vset(dst, p1->x - (p1->dmx * w), p1->y - (p1->dmy * w), 1,
                          1);
                dst++;
            }
            p0 = p1++;
...
static int nvg__expandFill(NVGcontext *ctx, float w, int lineJoin,
    ...
    for (i = 0; i < cache->npaths; i++) {
        ...
        if (fringe) {
            ...
            for (j = 0; j < path->count; ++j) {
                if (p1->flags & NVG_PT_BEVEL) {
                    ...
                    nvg__vset(dst, p1->x + (p1->dmx * woff),
                              p1->y + (p1->dmy * woff), 0.5f, 1);
                    dst++;
                }
                p0 = p1++;
            }
        ...
        if (fringe) {
            ...
            if (convex) {
                lw = woff; // This should generate the same vertex as fill inset
                           // above.
            ...
            for (j = 0; j < path->count; ++j) {
                if ((p1->flags & (NVG_PT_BEVEL | NVG_PR_INNERBEVEL)) != 0) {
                    ...
                    nvg__vset(dst, p1->x + (p1->dmx * lw),
                              p1->y + (p1->dmy * lw), lu, 1);
                    dst++;
                    nvg__vset(dst, p1->x - (p1->dmx * rw),
                              p1->y - (p1->dmy * rw), ru, 1);
                    dst++;
                }
                p0 = p1++;
...
void nvgArc(NVGcontext *ctx, float cx, float cy, float r, float a0, float a1,
    ...
    ndivs = nvg__maxi(
        1, nvg__mini((int)(nvg__absf(da) / (NVG_PI * 0.5f) + 0.5f), 5));
...
void nvgStroke(NVGcontext *ctx) {
    ...
    if (ctx->params.edgeAntiAlias && state->shapeAntiAlias)
        ...
        nvg__expandStroke(ctx, strokeWidth * 0.5f, state->lineCap,
...
static void nvg__flushTextTexture(NVGcontext *ctx) {
    ...
    if (fonsValidateTexture(ctx->fs, dirty)) {
        ...
        if (fontImage != 0) {
            ...
            ctx->params.renderUpdateTexture(ctx->params.userPtr, fontImage, x,
                                            y, w, h, data);
        }
    }
}
    ...
    if (ctx->fontImages[ctx->fontImageIdx + 1] != 0)
        ...
        ctx->fontImages[ctx->fontImageIdx + 1] =
            ctx->params.renderCreateTexture(ctx->params.userPtr,
                                            NVG_TEXTURE_ALPHA, iw, ih, 0, NULL);
    }
    ++ctx->fontImageIdx;
    ...
    while (
        ...
        for (i = 0; i < nrows; i++) {
            ...
            if (haling & NVG_ALIGN_LEFT)
                ...
                nvgText(ctx, x + breakRowWidth - row->width, y, row->start,
                        row->end);
            ...
            if (rowStart == NULL) {
                ...
                if ((type == NVG_CHAR || type == NVG_CJK_CHAR) &&
                    ...
                    if (breakEnd == rowStart) {
                        // The current word is longer than the row length, just
                        // break it from here.
                        ...
                        // Break the line from the end of the last word, and
